...
# --- 1. MANDATORY: PAGE CONFIG (MUST BE FIRST LINE) ---
st.set_page_config(
...
# --- 2. SOVEREIGN CONFIG & MISSION MEMORY ---
...
def clean_veda(text):
    # Silent filter: Blocks JSON errors and technical noise from reaching UI
...
# --- 5. PERMANENT SIDEBAR (Initialized before login) ---
...
if selected == "Medha (Chat)":
    ...
    if prompt := st.chat_input("Command VEDA..."):
        ...
        with st.chat_message("assistant"):
            ...
            # --- PHASE 2: SILENT ROTATION ---
            if not final_res:
                stealth_msgs = ["🧠 Performing Neural Analysis...", "🔱 Optimizing Logic Gates...", "🛰️ Accessing Failover Satellite..."]
                models = ["openai", "mistral", "claude", "llama"]
                for i, brain in enumerate(models):
                    status.markdown(f'<p class="thinking">{stealth_msgs[i % len(stealth_msgs)]}</p>', unsafe_allow_html=True)
                    try:
                        p_enc = urllib.parse.quote(prompt)
                        i_enc = urllib.parse.quote(IDENTITY)
                        # Randomized seed and stealth agent to bypass 429 queues
            ...
            status.empty()
            # Emergency Mission Fallback
            if not final_res:
                if any(x in prompt.lower() for x in ["who made you", "purpose", "creator", "karthik"]):
                    final_res = MISSION
                else:
                    final_res = "🔱 Sovereign Core is undergoing high-level optimization. Please re-command."
    ...
    if st.button("🚀 INITIATE VISUALIZATION", use_container_width=True):
        if vision:
            ...
            st.warning("Commander, provide a vision prompt.")
