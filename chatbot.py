...
st.set_page_config(page_title="VEDA 3.0", page_icon="🔱", layout="wide")
USER_NAME = "KARTHIK"
...
# Clean Identity: No technical jargon
IDENTITY = f"Your name is VEDA. You are talking to {USER_NAME}."
...
with st.sidebar:
    ...
    st.markdown(f"<h3 style='text-align: center; color: #FF8C00; margin-top:0;'>VEDA 3.0</h3>", unsafe_allow_html=True)
    
    st.markdown("---")
    # Simple Clock for Karthik
    st.markdown(f"""
    ...
    selected = option_menu(None, ["Medha (Chat)", "Srijan (Image Maker)"], 
                          icons=["chat-quote", "palette"], default_index=0)
...
if selected == "Medha (Chat)":
    st.markdown('<div class="orange-title">MEDHA</div>', unsafe_allow_html=True)
    
    for msg in st.session_state.chat_history:
        with st.chat_message(msg["role"]): st.markdown(msg["content"])

    if prompt := st.chat_input(f"Hello {USER_NAME}, ask Medha anything..."):
        ...
        with st.chat_message("assistant"):
            ...
            # Use the most stable engine for Medha
            if client:
                try:
            ...
            if not success:
                try:
                    ...
                    # 2026 Hyper-Link for Medha
                    r = requests.get(f"https://text.pollinations.ai/{q_enc}?model=openai", timeout=10)
                    if r.status_code == 200:
                        answer = r.text
                        success = True
                except: answer = "🔱 Connection reset. Try again."

            st.markdown(answer)
            st.session_state.chat_history.append({"role": "assistant", "content": answer})

elif selected == "Srijan (Image Maker)":
    st.markdown('<div class="orange-title">SRIJAN</div>', unsafe_allow_html=True)
    
    vision = st.text_input("Describe your vision:", placeholder="A futuristic city in India...")
    if st.button("🚀 CREATE"):
        if vision:
